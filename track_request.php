...
if(isset($_POST['search'])){
	$search = $_POST['id'];			
	$sql="select * from request_tbl where trans_id='$search'";
	$res=mysqli_query($con,$sql);
	$row = mysqli_fetch_assoc($res);
		if ($res) {
  			if (mysqli_num_rows($res) < 1) {  
	  			$sql="select * from requestreject_tbl where trans_id='$search'";
				$res=mysqli_query($con,$sql);
				$row = mysqli_fetch_assoc($res);
	  				if (mysqli_num_rows($res) < 1) {
...
if(isset($_POST['download'])){
	...
	$row = mysqli_fetch_assoc($res);
  	...
  	header("Content-Disposition: attachment; filename=".$row['docu_file']);
    header("Content-Type: application/octet-stream;");
    readfile("uploads/".$row['docu_file']);
      ...
      <div class="form-details form-details-active">
        <div class="input-group">
            ...
            <div class="search_wrap search_wrap_5">
                <div class="search_box">
                    <input type="text" class="input" name="id" id="id" placeholder="Enter Transaction ID" value="<?php echo $row['trans_id'];?>">
        ...
        <br>
		   ...
		   <div class="input-group">
            ...
            <label class="input" id="id"><?php echo $row['trans_id'];?></label>
          </div>
          <div class="input-group">
            <label>Name: </label>
		  ...
		  <div class="input-group">
            ...
            <label class="input" id="name"><?php echo $row['docu_type'];?></label>
          </div>
          <div class="input-group">
	 	...
	 	<div class="download_wrap download_wrap_5">
              <div class="download_box">
                  <input type="text" class="input" value="<?php echo $row['docu_file'];?>" >
                  <input type="submit" name="download" id="download" value="Download" class="btn2"/>
                  </div>
              </div>
		
		 
    	<p class="note">Note: Blank means your document file is not yet available</p>
